...
public sealed class MP2KConfig : Config
{
    ...
    internal readonly List<ScanResult> Results = [];

    internal readonly string Name;
    internal readonly long[] SongTableOffsets;
    public readonly long[] SongTableSizes;
    internal readonly int FrequencyIndex;
    ...
    public override HSLColor[]? Colors { get; protected set; }
    ...
    internal MP2KConfig(byte[] rom, bool mainPlaylistFirst)
    {
        ...
        var scanner = new MP2KScanner(rom);
        Results = scanner.Scan();

        SongTableOffsets = new long[Results.Count];
        SongTableSizes = new long[Results.Count];
        for (int i = 0; i < Results.Count; i++)
        {
            SongTableOffsets[i] = Results[i].SongTableInfo.Position;
            SongTableSizes[i] = Results[i].SongTableInfo.Count;
        }
        try
        {
            ...
            void Load(YamlMappingNode gameToLoad)
            {
                ...
                if (gameToLoad.Children.TryGetValue(nameof(FrequencyIndex), out node))
                {
                    sampleRateNode = node;
                }
            ...
            SongTableOffsets = new long[numSongTables];
            ...
            {
                throw new BetterKeyNotFoundException(nameof(FrequencyIndex), null);
            }
            FrequencyIndex = (int)ConfigUtils.ParseValue(nameof(FrequencyIndex), sampleRateNode.ToString(), 0, MP2KUtils.FrequencyTable.Length - 1);
