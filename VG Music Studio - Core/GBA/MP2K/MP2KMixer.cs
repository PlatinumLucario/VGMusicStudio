...
public sealed class MP2KMixer : Mixer
{
    ...
    internal void Process(bool output, bool recording)
    {
        ...
        for (int i = 0; i < _pcm8Channels.Length; i++)
        {
            ...
            {
                var index = c.Owner.Index;
                c.Process(_trackBuffers[index]);
                Reverbs[index].Process(_trackBuffers[index], SamplesPerBuffer);
                //if (c.Owner is not null)
                //{
                //	Reverbs[i].Process(_trackBuffers[c.Owner.Index], SamplesPerBuffer);
                //}
            }
        }
        ...
        //for (int i = 0; i < _trackBuffers.Length; i++)
        //{
        //    Reverbs[i].Process(_trackBuffers[i], SamplesPerBuffer);
        //}
